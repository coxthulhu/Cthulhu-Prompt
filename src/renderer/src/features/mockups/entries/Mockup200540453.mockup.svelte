<script lang="ts">
  ...
  const completedPrompts: MockPrompt[] = [
    prompt('completed-sidebar-audit', 'Audit the existing sidebar'),
    prompt('completed-workflow-notes', 'Summarize the prompt workflow'),
    prompt('completed-data-model', 'Review the prompt data model'),
    prompt('completed-tree-states', 'Document current tree states')
  ]
...
<main class="sidebar-base-stage" data-testid="app-sidebar-base-mockup">
  <aside class="mock-sidebar" aria-label="Cthulhu Prompt sidebar">
    ...
    <div class="accordion-stack">
      <section class="accordion-section completed-section" style="flex: 0 0 178px;">
        <button
          class="accordion-header"
          type="button"
          aria-expanded="true"
          aria-label="Collapse completed prompts"
        >
          <span class="accordion-icon completed-icon">
            <CheckCircle2 size={16} strokeWidth={1.8} aria-hidden="true" />
          </span>
          <span class="accordion-title">Completed prompts</span>
          <span class="accordion-count">4</span>
          <ChevronDown class="accordion-chevron" size={17} aria-hidden="true" />
        </button>
          ...
          <div class="prompt-tree">
            {#each completedPrompts as promptEntry, promptIndex (promptEntry.id)}
              {@render PromptRow(
                promptEntry,
                0,
                promptIndex === completedPrompts.length - 1
              )}
            {/each}
          </div>
        </div>
      ...
      <section class="accordion-section active-section" style="flex: 1 1 auto;">
        <button
          class="accordion-header"
          type="button"
          aria-expanded="true"
          aria-label="Collapse active prompts"
        >
          <span class="accordion-icon active-icon">
            <Circle size={16} strokeWidth={1.8} aria-hidden="true" />
          </span>
          <span class="accordion-title">Active prompts</span>
