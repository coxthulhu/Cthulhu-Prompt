<script lang="ts">
  ...
  const addPromptButtonStyle = (key: string, highlighted = false) =>
    `height:${highlighted ? '34px' : '30px'};border:${highlighted ? '1px dashed var(--ui-accent-hover-border)' : `1px solid ${isHovered(key) ? 'var(--ui-accent-hover-border)' : 'var(--ui-neutral-normal-border)'}`};border-radius:${highlighted ? '10px' : '999px'};background:${highlighted ? 'linear-gradient(to bottom,var(--ui-accent-hover-surface),var(--ui-neutral-normal-surface))' : isHovered(key) ? 'var(--ui-accent-hover-surface)' : 'var(--ui-neutral-normal-surface)'};color:var(--ui-normal-text);display:inline-flex;align-items:center;gap:7px;padding:${highlighted ? '0 15px' : '0 13px'};font-size:13px;font-weight:700;cursor:pointer;box-shadow:${highlighted ? '0 0 0 1px var(--ui-accent-normal-border),0 10px 24px var(--ui-shadow-raised),inset 0 1px 0 var(--ui-card-nested-raised-inset-highlight),inset 0 -1px 0 var(--ui-shadow-inset)' : 'inset 0 1px 0 var(--ui-card-nested-inset-highlight)'};transition:background-color 120ms ease,border-color 120ms ease,color 120ms ease,box-shadow 120ms ease;`

  const addPromptSeparatorStyle = (highlighted = false) =>
    highlighted
      ? 'height:30px;border:1px dashed var(--ui-accent-hover-border);border-radius:10px;background:linear-gradient(to bottom,var(--ui-accent-normal-surface),var(--ui-neutral-muted-surface));box-shadow:0 0 0 1px var(--ui-accent-normal-border),0 8px 18px var(--ui-shadow-raised),inset 0 1px 0 var(--ui-card-nested-raised-inset-highlight);'
