<script lang="ts">
  ...
  const createPrompt = (
    ...
    categoryLabel: id.includes('discovery')
      ? 'Research'
  ...
  let categoryDialogOpen = $state(true)
        ...
        <div class="base-prompt-title-main">
          ...
          <div class="base-title-copy">
            ...
            <div class="base-metadata-row">
              ...
              {@render SeparatorDot()}
              <span
                style="align-items:center;background:var(--ui-neutral-action-fill);border:1px solid var(--ui-neutral-muted-border);border-radius:999px;color:var(--ui-hoverable-text);display:inline-flex;gap:4px;line-height:14px;padding:2px 7px;"
              >
      ...
      <div class="base-root-title-row">
        ...
        <div style="align-items:center;display:flex;gap:8px;">
          <button
            ...
            style="align-items:center;background:var(--ui-neutral-action-fill);border:1px solid var(--ui-neutral-normal-border);border-radius:var(--cthulhu-ui-radius-control);color:var(--ui-normal-text);display:inline-flex;font-size:13px;font-weight:600;gap:7px;height:36px;padding:0 12px;"
          >
...
<div
  class="base-template-dialog-layer"
  role="presentation"
  data-testid="manage-categories-dialog-layer"
>
  <div
    class="base-template-dialog"
    role="dialog"
  ...
  >
    <header class="base-template-dialog-header" style="padding:0 0 18px;">
      <div class="base-template-dialog-heading" style="gap:14px;">
        <div
          class="base-template-dialog-icon"
          style="background:var(--ui-accent-action-fill);border:1px solid var(--ui-accent-muted-border);border-radius:var(--cthulhu-ui-radius-card);color:var(--ui-normal-text);height:42px;width:42px;"
        >
      ...
      {@render IconButton(X, 'Close', { onclick: () => (categoryDialogOpen = false) })}
    </header>

    {@render Separator()}

    <div class="base-template-dialog-body" style="padding-top:16px;">
      <section
        aria-label="Add category"
        style="background:var(--ui-card-normal-surface);border:1px solid var(--ui-card-nested-border);border-radius:var(--cthulhu-ui-radius-card);padding:14px;"
      >
        <div style="align-items:center;display:flex;justify-content:space-between;margin-bottom:11px;">
          <strong style="color:var(--ui-normal-text);font-size:14px;font-weight:650;">Add category</strong>
        </div>
        <div style="align-items:end;display:grid;gap:10px;grid-template-columns:minmax(0,1fr) minmax(0,1fr) auto;">
          <label style="color:var(--ui-muted-text);display:grid;font-size:12px;gap:6px;line-height:16px;">
            <span>Name</span>
            <input
              aria-label="New category name"
              value="Release Notes"
              style="background:var(--ui-card-solid-surface);border:1px solid var(--ui-neutral-normal-border);border-radius:var(--cthulhu-ui-radius-control);box-sizing:border-box;color:var(--ui-normal-text);height:38px;min-width:0;padding:0 10px;width:100%;"
            />
          </label>
          <label style="color:var(--ui-muted-text);display:grid;font-size:12px;gap:6px;line-height:16px;">
            <span>ID</span>
            <input
              ...
              value="release-notes"
              style="background:var(--ui-card-solid-surface);border:1px solid var(--ui-neutral-normal-border);border-radius:var(--cthulhu-ui-radius-control);box-sizing:border-box;color:var(--ui-normal-text);font-family:Consolas,monospace;height:38px;min-width:0;padding:0 10px;width:100%;"
            />
          </label>
          <button
            type="button"
            style="align-items:center;background:var(--ui-accent-action-fill);border:1px solid var(--ui-accent-normal-border);border-radius:var(--cthulhu-ui-radius-control);color:var(--ui-normal-text);display:inline-flex;font-size:13px;font-weight:650;gap:7px;height:38px;padding:0 13px;white-space:nowrap;"
          >
            <Plus size={16} aria-hidden="true" />
            Add Category
          </button>
      ...
      <div style="align-items:end;display:flex;justify-content:space-between;padding:18px 2px 8px;">
        <strong style="color:var(--ui-normal-text);font-size:14px;font-weight:650;">Categories</strong>
        <span style="color:var(--ui-muted-text);font-size:12px;">7 categories</span>
      </div>

      <div
        aria-label="Category list"
        style="border:1px solid var(--ui-card-nested-border);border-radius:var(--cthulhu-ui-radius-card);max-height:min(330px,calc(100vh - 390px));overflow-y:auto;"
      >
        <div
          style="align-items:center;border-bottom:1px solid var(--ui-card-nested-border);display:grid;gap:12px;grid-template-columns:minmax(0,1fr) minmax(0,0.9fr) 76px;padding:8px 12px;"
        >
          <span style="color:var(--ui-muted-text);font-size:11px;font-weight:650;letter-spacing:.04em;text-transform:uppercase;">Name</span>
          <span style="color:var(--ui-muted-text);font-size:11px;font-weight:650;letter-spacing:.04em;text-transform:uppercase;">ID</span>
          <span></span>
        </div>
        ...
        ] as category, index (category.id)}
          <div
            style={`align-items:center;${index < 2 ? 'border-bottom:1px solid var(--ui-card-nested-border);' : ''}display:grid;gap:12px;grid-template-columns:minmax(0,1fr) minmax(0,0.9fr) 76px;min-height:48px;padding:0 12px;`}
          >
            <span style="align-items:center;color:var(--ui-normal-text);display:flex;font-size:13px;font-weight:550;gap:9px;min-width:0;">
              <span style="background:var(--ui-accent-normal-border);border-radius:999px;height:7px;width:7px;"></span>
              {category.name}
            </span>
            <code style="color:var(--ui-muted-text);font-size:12px;">{category.id}</code>
            <span style="display:flex;gap:4px;justify-content:flex-end;">
              {@render IconButton(Pencil, `Edit ${category.name}`, { size: 'compact' })}
              {@render IconButton(Trash2, `Delete ${category.name}`, {
                size: 'compact',
                hoverVariant: 'danger'
              })}
            </span>
        ...
        <div
          style="background:var(--ui-neutral-subtle-action-hover-fill);border-bottom:1px solid var(--ui-card-nested-border);border-top:1px solid var(--ui-card-nested-border);display:grid;gap:10px;grid-template-columns:minmax(0,1fr) minmax(0,0.9fr) auto;min-height:68px;padding:10px 12px;"
        >
          <input
            aria-label="Category name"
            value="Review"
            style="background:var(--ui-card-solid-surface);border:1px solid var(--ui-neutral-normal-border);border-radius:var(--cthulhu-ui-radius-control);box-sizing:border-box;color:var(--ui-normal-text);height:36px;min-width:0;padding:0 9px;width:100%;"
          />
          <input
            aria-label="Category ID"
            value="review"
            style="background:var(--ui-card-solid-surface);border:1px solid var(--ui-neutral-normal-border);border-radius:var(--cthulhu-ui-radius-control);box-sizing:border-box;color:var(--ui-normal-text);font-family:Consolas,monospace;height:36px;min-width:0;padding:0 9px;width:100%;"
          />
          <span style="display:flex;gap:6px;justify-content:flex-end;">
            <button
              type="button"
              style="align-items:center;background:var(--ui-accent-action-fill);border:1px solid var(--ui-accent-normal-border);border-radius:var(--cthulhu-ui-radius-control);color:var(--ui-normal-text);display:inline-flex;font-size:12px;font-weight:650;gap:5px;height:36px;padding:0 10px;"
            >
              <Check size={14} aria-hidden="true" />
              Save
            </button>
            {@render IconButton(X, 'Cancel editing', { size: 'default' })}
          </span>
        </div>
        ...
        {#each [
          ...
          >
            <span style="align-items:center;color:var(--ui-normal-text);display:flex;font-size:13px;font-weight:550;gap:9px;min-width:0;">
              <span style="background:var(--ui-secondary-icon-glyph);border-radius:999px;height:7px;width:7px;"></span>
              {category.name}
            </span>
            <code style="color:var(--ui-muted-text);font-size:12px;">{category.id}</code>
            <span style="display:flex;gap:4px;justify-content:flex-end;">
    ...
    <footer class="base-template-dialog-footer" style="padding-top:16px;">
      <button type="button" class="base-dialog-confirm-button" onclick={() => (categoryDialogOpen = false)}>Done</button>
    </footer>
  </div>
</div>
{/if}
