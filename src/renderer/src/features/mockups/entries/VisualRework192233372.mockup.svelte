<script lang="ts">
  ...
  const getEditorUri = (documentId: string) =>
    monaco.Uri.file(`/cthulhu-prompt/mockups/visual-rework-192233372/${documentId}.md`)
  ...
  let settingsMenuOpenFolderId = $state<string | null>('base-implementation')

  const addFolderSetting = (setting: MockFolderSetting) => {
    setting.isPresent = true
    settingsMenuOpenFolderId = null
  }
</script>
      ...
      <div class="base-editor-body">
        ...
        <div class="base-folder-settings">
          <div class="base-settings-toolbar">
            <div class="base-settings-toolbar-label">
              <Settings size={14} aria-hidden="true" />
              <span>Folder settings</span>
              <span class="base-settings-count">
                {folder.settings.filter((setting) => setting.isPresent).length} of {folder.settings.length}
              </span>
            </div>

            <div class="base-add-setting-control">
              <button
                class="base-add-setting-trigger"
                type="button"
                aria-haspopup="menu"
                aria-expanded={settingsMenuOpenFolderId === folder.id}
                disabled={folder.settings.every((setting) => setting.isPresent)}
                onclick={() => {
                  settingsMenuOpenFolderId =
                    settingsMenuOpenFolderId === folder.id ? null : folder.id
                }}
              >
              ...
              {#if settingsMenuOpenFolderId === folder.id}
                <div class="base-add-setting-menu" role="menu" aria-label="Available folder settings">
                  <div class="base-add-setting-menu-heading">Available settings</div>
                  {#each folder.settings.filter((setting) => !setting.isPresent) as setting (setting.id)}
                    <button
                      type="button"
                      role="menuitem"
                      onclick={() => addFolderSetting(setting)}
                    >
                      <span class="base-add-setting-menu-icon">
                        <Plus size={14} aria-hidden="true" />
                      </span>
                      <span class="base-add-setting-menu-copy">
          ...
          {#each folder.settings.filter((setting) => setting.isPresent) as setting (setting.id)}
            <section
              class="base-settings-section withTopBorder"
...
<main class="base-prompt-folder-mockup" data-testid="visual-rework-192233372-mockup">
...
<style>
  ...
  .base-add-setting-control {
    position: relative;
  }

  .base-add-setting-trigger {
    align-items: center;
    background: transparent;
    border: 1px solid transparent;
    border-radius: var(--cthulhu-ui-radius-control);
  ...
  .base-add-setting-trigger:hover,
  .base-add-setting-trigger[aria-expanded='true'] {
    background: var(--ui-neutral-subtle-action-hover-fill);
    border-color: var(--ui-neutral-normal-border);
    color: var(--ui-hoverable-text);
  }

  .base-add-setting-trigger:disabled {
    cursor: default;
    opacity: 0.45;
  }

  .base-add-setting-menu {
    background: var(--ui-editor-normal-surface);
    border: 1px solid var(--ui-neutral-emphasis-border);
    border-radius: var(--cthulhu-ui-radius-card);
    box-shadow: 0 10px 30px color-mix(in srgb, var(--ui-editor-content-surface) 70%, transparent);
    box-sizing: border-box;
    display: grid;
    min-width: 330px;
    overflow: hidden;
  ...
  .base-add-setting-menu > button:hover,
  .base-add-setting-menu > button:focus-visible {
    background: var(--ui-neutral-subtle-action-hover-fill);
  }

  .base-add-setting-menu-icon {
    align-items: center;
  ...
  .base-add-setting-menu-copy {
    display: grid;
    gap: 3px;
    min-width: 0;
  }

  .base-add-setting-menu-copy strong {
    font-size: 12px;
    line-height: 16px;
  }

  .base-add-setting-menu-copy > span {
    color: var(--ui-muted-text);
    font-size: 11px;
    line-height: 15px;
  }
