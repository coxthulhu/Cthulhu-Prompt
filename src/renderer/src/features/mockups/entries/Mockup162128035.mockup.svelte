...
{#if dialog}
  <div class="dialog-layer text-base" role="presentation" use:mountDialog onclick={(event) => { if (event.target === event.currentTarget && dialog === 'error') closeDialog() }}>
    <div class="home-dialog" style={dialog === 'welcome' ? 'max-width: 800px; padding: 0; border-radius: 14px; overflow: hidden;' : undefined} data-create={dialog === 'create'} data-welcome={dialog === 'welcome'} role="dialog" aria-modal="true" aria-label={dialog === 'create' ? 'Create Workspace' : dialog === 'welcome' ? 'Welcome' : 'Failed to Open Workspace'} tabindex="-1">
      <div class="dialog-header" style={dialog === 'welcome' ? 'padding: 22px 26px 18px; background: var(--ui-card-normal-surface);' : undefined}>
        <div class="dialog-heading">
          ...
          <div class="dialog-heading-text">
            ...
            {#if dialog === 'welcome'}<p class="text-sm leading-5" style="margin: 4px 0 0; color: var(--ui-muted-text);">Your first prompt, from workspace to clipboard.</p>{/if}
      ...
      {:else if dialog === 'welcome'}
        <div class="text-sm leading-6" style="min-height: 0; overflow-y: auto; padding: 0 28px; color: var(--ui-hoverable-text);">
          <ol style="list-style: none; padding: 0; margin: 0;">
            <li style="display: grid; grid-template-columns: 44px minmax(0, 1fr); gap: 18px; padding: 20px 0; border-bottom: 1px solid var(--ui-neutral-muted-border);">
              <span class="text-xl leading-10 font-semibold" style="width: 42px; height: 42px; text-align: center; border: 1px solid var(--ui-accent-muted-border); border-radius: 50%; color: var(--ui-normal-text); background: var(--ui-accent-action-fill);">1</span>
              <div>
                <h2 class="text-base leading-6 font-semibold" style="display: flex; align-items: center; gap: 9px; margin: 0 0 6px; color: var(--ui-normal-text);"><Home size={19} aria-hidden="true" />Create a workspace</h2>
                <p style="margin: 0;">Choose a folder on your computer. Your tasks and templates live here as simple Markdown files, ready to commit to source control.</p>
                <div class="text-xs leading-5" style="display: flex; align-items: center; gap: 8px; margin-top: 10px; color: var(--ui-muted-text);"><span style="display: flex; gap: 10px; padding: 5px 9px; border: 1px solid var(--ui-neutral-muted-border); border-radius: 5px;"><Home size={14} /><FileText size={14} /><Layers size={14} /></span>Use the left-hand icon bar to switch screens.</div>
              </div>
            </li>
            <li style="display: grid; grid-template-columns: 44px minmax(0, 1fr); gap: 18px; padding: 18px 0; border-bottom: 1px solid var(--ui-neutral-muted-border);">
              <span class="text-xl leading-10 font-semibold" style="width: 42px; height: 42px; text-align: center; border: 1px solid var(--ui-neutral-normal-border); border-radius: 50%; color: var(--ui-muted-text);">2</span>
              <div>
                <h2 class="text-base leading-6 font-semibold" style="display: flex; align-items: center; gap: 9px; margin: 0 0 6px; color: var(--ui-normal-text);"><Layers size={19} aria-hidden="true" />Build a prompt template<span class="text-xs leading-5 font-normal" style="margin-left: auto; padding: 0 8px; border: 1px solid var(--ui-neutral-muted-border); border-radius: 4px; color: var(--ui-muted-text);">Optional</span></h2>
                ...
                <div class="text-xs leading-5" style="display: flex; flex-wrap: wrap; gap: 6px; margin-top: 9px; color: var(--ui-muted-text);">
            ...
            <li style="display: grid; grid-template-columns: 44px minmax(0, 1fr); gap: 18px; padding: 18px 0; border-bottom: 1px solid var(--ui-neutral-muted-border);">
              <span class="text-xl leading-10 font-semibold" style="width: 42px; height: 42px; text-align: center; border: 1px solid var(--ui-neutral-normal-border); border-radius: 50%; color: var(--ui-muted-text);">3</span>
              <div>
                <h2 class="text-base leading-6 font-semibold" style="display: flex; align-items: center; gap: 9px; margin: 0 0 6px; color: var(--ui-normal-text);"><FileText size={19} aria-hidden="true" />Add a prompt task</h2>
                <p style="margin: 0;">Open Task Prompts and add the change you want your AI to make. Include context, requirements, and details. Name your tasks and group them into custom categories.</p>
                <div class="text-xs leading-5" style="display: flex; flex-wrap: wrap; align-items: center; gap: 7px; margin-top: 9px; color: var(--ui-muted-text);">Backlog<ArrowRight size={12} />Todo<ArrowRight size={12} />In Progress<ArrowRight size={12} /><span style="color: var(--ui-normal-text);">Completed</span></div>
              </div>
            </li>
            <li style="display: grid; grid-template-columns: 44px minmax(0, 1fr); gap: 18px; padding: 18px 0 22px;">
              <span class="text-xl leading-10 font-semibold" style="width: 42px; height: 42px; text-align: center; border: 1px solid var(--ui-neutral-normal-border); border-radius: 50%; color: var(--ui-muted-text);">4</span>
              <div>
                <h2 class="text-base leading-6 font-semibold" style="display: flex; align-items: center; gap: 9px; margin: 0 0 6px; color: var(--ui-normal-text);"><Zap size={19} aria-hidden="true" />Pick a template. Copy and go.</h2>
                <p style="margin: 0;">Use the quick template button to assign a template, or choose no template, and copy the combined text. Paste it into your AI tool and send it off.</p>
                <div class="text-xs leading-5" style="display: flex; align-items: center; gap: 8px; margin-top: 9px; color: var(--ui-muted-text);"><span style="display: flex; padding: 2px; border: 1px solid var(--ui-neutral-normal-border); border-radius: 3px; color: var(--ui-normal-text);"><Check size={12} /></span>When the AI finishes, check off the task to move it to Completed.</div>
              </div>
            </li>
          </ol>
        </div>
      ...
      {/if}
      <div class="separator"></div>
      <div class="dialog-footer" style={dialog === 'welcome' ? 'padding: 16px 26px; background: var(--ui-card-normal-surface); flex-wrap: wrap;' : undefined}>
