...
<main class="base-prompt-folder-mockup" data-testid="base-prompt-folder-mockup">
  ...
  <div class="base-content-viewport" bind:this={viewport} bind:clientHeight={viewportHeight} onscroll={updateBreadcrumb}>
    <section class="base-root-header" style="gap: 14px; grid-template-rows: 60px 44px; padding-top: 16px;">
      <div class="base-root-title-row" style="align-items: center; justify-content: flex-start;">
        <div class="base-root-title-block">
          {@render IconCell(FileText)}
          <div class="base-root-title-stack">
            <div class="base-root-title-line" style="align-items: center; gap: 16px;">
              <h1 class="text-3xl leading-9">{rootTitle}</h1>
              <button type="button" class="base-management-add-button text-sm leading-5" title="Rename folder"
                style="width: auto; height: 34px; margin: 0; padding: 0 10px; gap: 7px; background: var(--ui-neutral-normal-surface);"
      ...
      <div style="display: flex; align-items: center; justify-content: space-between; gap: 16px; margin: 0 24px; min-width: 0;">
      <div class="base-filter-bar" role="group" aria-label="Filter prompts"
        style="padding: 4px; border: 1px solid var(--ui-neutral-muted-border); border-radius: 10px; background: var(--ui-card-solid-surface); height: 44px; gap: 4px;">
        {#each groups as group (group)}
          <button class="text-sm leading-5" class:active={screenMode === group} type="button" aria-pressed={screenMode === group}
            style={`height: 34px; display: flex; align-items: center; gap: 8px; padding: 0 12px; margin: 0; border: 0; border-radius: 6px; background: ${screenMode === group ? 'var(--ui-accent-action-fill)' : 'var(--ui-ghost-surface)'}; box-shadow: ${screenMode === group ? 'inset 0 0 0 1px var(--ui-accent-normal-border)' : 'none'};`}
            onclick={() => selectGroup(group)}>
            {group} <span class="text-xs leading-4.5" style="top: 0;">{allPrompts.filter((prompt) => matchesGroup(prompt, group)).length}</span>
          </button>
        {/each}
      </div>
      <div class="base-icon-button-bar" style="height: 44px; padding: 4px; border: 1px solid var(--ui-neutral-muted-border); border-radius: 10px; background: var(--ui-card-solid-surface); box-sizing: border-box; gap: 4px;">
        <button type="button" class="base-management-add-button text-sm leading-5" title="Manage categories"
          style="width: auto; height: 34px; margin: 0; padding: 0 12px; border: 0; background: var(--ui-neutral-normal-surface); gap: 8px;"
          onclick={() => openManageCategories()}>
          <FolderCog size={18} />
          Categories
        </button>
        <span style="height: 20px; width: 1px; background: var(--ui-neutral-normal-border); margin: 0 3px;"></span>
