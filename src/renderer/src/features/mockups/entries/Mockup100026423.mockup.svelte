<script lang="ts">
  ...
  const categoryRows = [
    { name: 'Implementation', folder: 'Product Work', count: 12 },
    { name: 'Verification', folder: 'Product Work', count: 8 },
    { name: 'Planning', folder: 'Product Work', count: 6 },
    { name: 'Code Review', folder: 'Engineering', count: 9 },
    { name: 'Documentation', folder: 'Engineering', count: 4 },
    { name: 'Release Notes', folder: 'Engineering', count: 3 }
  ]
  // Side effect: size the standard dialog for a two-panel category editor.
  const sizeCategoryDialog = (node: HTMLElement) => {
    const dialog = node.closest<HTMLElement>('[role="dialog"]')
    if (dialog) dialog.style.width = 'min(1080px, calc(100vw - 32px))'
  }
  // Side effect: mount a fixed-height Monaco preview and dispose its resources on close.
  const mountCategoryDescription = (node: HTMLElement) => {
    const model = monaco.editor.createModel([
      '# Implementation', '',
      'Prompts for turning an approved plan into working code.', '',
      '## Scope',
      '- Build features using the existing architecture.',
      '- Keep changes focused on the approved requirements.',
      '- Include meaningful verification for each change.', '',
      '## Working agreement',
      'Read the relevant code before editing.',
      'Preserve unrelated changes in the working tree.',
      'Summarize the outcome and verification when finished.'
    ].join('\n'), 'markdown')
    const editor = monaco.editor.create(node, {
      model, automaticLayout: true, ariaLabel: 'Category description',
      fontSize: editorSizing.fontSize, lineHeight: lineHeightPx,
      minimap: { enabled: false }, lineNumbers: 'on', lineNumbersMinChars: 3,
      glyphMargin: false, wordWrap: 'on', scrollBeyondLastLine: false,
      overviewRulerBorder: false, padding: { top: 12, bottom: 12 },
      scrollbar: { handleMouseWheel: true, alwaysConsumeMouseWheel: true }
    })
  ...
  <div class="base-content-viewport" bind:this={viewport} bind:clientHeight={viewportHeight} onscroll={updateBreadcrumb}>
    <section class="base-root-header">
      <div class="base-root-title-row">
        ...
        <div class="base-icon-button-bar">
          <button type="button" class="text-sm leading-5" style="display: flex; align-items: center; gap: 8px; padding: 9px 12px; border: 1px solid var(--ui-neutral-normal-border); border-radius: var(--cthulhu-ui-radius-control); background: var(--ui-neutral-action-fill); color: var(--ui-normal-text);" onclick={() => categoriesOpen = true}><Settings size={16} />Manage Categories</button>
...
<Dialog bind:open={categoriesOpen} icon={Settings} title="Manage Categories" subtitle="Organize categories and edit their details." submitText="Save Changes" cancelText="Close" cancelFirst>
    <div use:sizeCategoryDialog style="display: grid; grid-template-columns: 250px minmax(0, 1fr); height: min(570px, calc(100vh - 230px)); min-height: 280px;">
      <aside style="display: flex; flex-direction: column; min-height: 0; border-right: 1px solid var(--ui-neutral-normal-border); padding: 20px 16px 20px 0;">
        ...
        <div style="flex: 1; overflow-y: auto;">
          {#each categoryRows as category, index (category.name)}
            <button type="button" aria-pressed={index === 0} style="display: flex; align-items: center; gap: 10px; width: 100%; text-align: left; padding: 12px 10px; margin-bottom: 5px; border: 1px solid {index === 0 ? 'var(--ui-accent-muted-border)' : 'transparent'}; border-radius: var(--cthulhu-ui-radius-control); background: {index === 0 ? 'var(--ui-accent-action-fill)' : 'var(--ui-ghost-surface)'}; color: var(--ui-normal-text);">
              <Folder size={17} style="flex-shrink: 0; color: var(--ui-secondary-icon-glyph);" />
              <span style="display: grid; gap: 3px; flex: 1;">
                <span class="text-sm leading-5" style="font-weight: 600;">{category.name}</span>
                <span class="text-xs leading-4" style="color: var(--ui-muted-text);">{category.folder}</span>
              </span>
              <span class="text-xs leading-4" style="color: var(--ui-muted-text);">{category.count}</span>
            </button>
          {/each}
        </div>
        <button type="button" class="text-sm leading-5" style="display: flex; align-items: center; justify-content: center; gap: 8px; min-height: 38px; margin-top: 14px; border: 1px solid var(--ui-neutral-normal-border); border-radius: var(--cthulhu-ui-radius-control); background: var(--ui-neutral-action-fill); color: var(--ui-normal-text);"><Plus size={16} />New Category</button>
      </aside>
      <section aria-label="Selected category" style="min-width: 0; overflow-y: auto; padding: 20px 8px 24px 24px;">
        <div style="display: flex; align-items: center; gap: 8px; margin-bottom: 20px;">
          <FolderOpen size={17} style="color: var(--ui-secondary-icon-glyph);" />
          <span class="text-xs leading-4" style="color: var(--ui-muted-text);">Product Work</span>
          <ChevronRight size={13} style="color: var(--ui-muted-text);" />
          <span class="text-xs leading-4" style="color: var(--ui-normal-text);">Implementation</span>
        </div>
        <div style="display: grid; gap: 18px;">
          <label style="display: grid; gap: 6px;">
            <span class="text-sm leading-5" style="font-weight: 600;">Category Name <span style="color: var(--ui-muted-text);">*</span></span>
            <input class="text-sm leading-5" value="Implementation" required aria-describedby="category-name-help-423" style="height: 38px; padding: 0 12px; color: var(--ui-normal-text); background: var(--ui-neutral-field-surface); border: 1px solid var(--ui-neutral-normal-border); border-radius: var(--cthulhu-ui-radius-control);" />
            <span id="category-name-help-423" class="text-xs leading-4" style="color: var(--ui-muted-text);">Required. Names must be unique in this prompt folder, ignoring case and surrounding spaces.</span>
          </label>
          <label style="display: grid; gap: 6px;">
            <span class="text-sm leading-5" style="font-weight: 600;">Short Description</span>
            <input class="text-sm leading-5" value="Build and refine approved product changes." style="height: 38px; padding: 0 12px; color: var(--ui-normal-text); background: var(--ui-neutral-field-surface); border: 1px solid var(--ui-neutral-normal-border); border-radius: var(--cthulhu-ui-radius-control);" />
            <span class="text-xs leading-4" style="color: var(--ui-muted-text);">A brief summary of the prompts in this category.</span>
          </label>
          <div style="display: grid; gap: 6px;">
            <span class="text-sm leading-5" style="font-weight: 600;">Category Description</span>
            <span class="text-xs leading-4" style="color: var(--ui-muted-text);">Add context and guidance for this category. Supports Markdown.</span>
            <div style="border: 1px solid var(--ui-neutral-normal-border); border-radius: var(--cthulhu-ui-radius-control); overflow: hidden;">
              <div use:mountCategoryDescription style="height: 180px; width: 100%;"></div>
            </div>
          </div>
          <div style="border-top: 1px solid var(--ui-neutral-normal-border); padding-top: 16px; display: grid; grid-template-columns: 1fr 1fr; gap: 18px;">
            <label style="display: grid; gap: 6px;">
              <span class="text-sm leading-5" style="font-weight: 600;">Default Prompt Status</span>
              <select class="text-sm leading-5" style="height: 36px; padding: 0 10px; border: 1px solid var(--ui-neutral-normal-border); border-radius: var(--cthulhu-ui-radius-control); background: var(--ui-neutral-field-surface); color: var(--ui-normal-text);"><option>Todo</option><option>Backlog</option></select>
              <span class="text-xs leading-4" style="color: var(--ui-muted-text);">Applied to new prompts in this category.</span>
            </label>
            <label style="display: grid; gap: 6px;">
              <span class="text-sm leading-5" style="font-weight: 600;">Default Template</span>
              <select class="text-sm leading-5" style="height: 36px; padding: 0 10px; border: 1px solid var(--ui-neutral-normal-border); border-radius: var(--cthulhu-ui-radius-control); background: var(--ui-neutral-field-surface); color: var(--ui-normal-text);"><option>No template</option><option>Draft Implementation Plan</option></select>
              <span class="text-xs leading-4" style="color: var(--ui-muted-text);">The starting template for new prompts.</span>
            </label>
          </div>
        </div>
      </section>
