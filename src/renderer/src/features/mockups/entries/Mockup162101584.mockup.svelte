<script lang="ts">
  ...
  import {
    ...
    FolderOpen, FolderPlus, Folders, FolderSymlink, Home, Layers, Zap, ArrowRight, Check, X
  } from 'lucide-svelte'
...
{#if dialog}
  <div class="dialog-layer text-base" role="presentation" use:mountDialog onclick={(event) => { if (event.target === event.currentTarget && dialog === 'error') closeDialog() }}>
    <div class="home-dialog" style={dialog === 'welcome' ? 'max-width: 860px; padding: 24px; border-radius: 16px;' : undefined} data-create={dialog === 'create'} data-welcome={dialog === 'welcome'} role="dialog" aria-modal="true" aria-label={dialog === 'create' ? 'Create Workspace' : dialog === 'welcome' ? 'Welcome' : 'Failed to Open Workspace'} tabindex="-1">
      <div class="dialog-header">
        <div class="dialog-heading">
          ...
          <div class="dialog-heading-text">
            ...
            {#if dialog === 'welcome'}<p class="text-sm leading-5" style="margin: 4px 0 0; color: var(--ui-muted-text);">Your first prompt, in four steps.</p>{/if}
      ...
      {:else if dialog === 'welcome'}
        <div class="welcomeContent text-sm leading-5" style="padding: 8px 0 16px;">
          <ol style="list-style: none; padding: 0; margin: 0;">
            {#each [
              { number: '01', icon: Home, title: 'Make a workspace', description: 'Create a folder on your computer for your tasks and templates. Everything is saved as simple Markdown files, ready for source control.', detail: 'Use the icons on the left to switch between screens.' },
              { number: '02', icon: Layers, title: 'Make a prompt template', description: 'Open Templates and write a reusable workflow: investigate a bug, plan a feature with Q&A, or create mockups. Your task adds the specifics.', detail: 'Optional — start with a task and add templates later.' },
              { number: '03', icon: FileText, title: 'Make a prompt task', description: 'Open Prompt Tasks and add the change you want your AI to make. Give it a name, then add context, requirements, and details.', detail: 'Organize tasks in custom categories and track their status.' },
              { number: '04', icon: Zap, title: 'Pick a template. Copy and go.', description: 'Use Quick Template on a task to choose its template, or use none. Copy the combined prompt and paste it into your AI tool.', detail: 'When it’s done, check off the task to move it to Completed.' }
            ] as step, index (step.number)}
              <li style="display: grid; grid-template-columns: 36px minmax(0, 1fr); gap: 16px; padding: 20px 0; border-bottom: {index < 3 ? '1px solid var(--ui-neutral-muted-border)' : 'none'};">
                <span class="text-xl leading-7 font-light" style="color: var(--ui-accent-link-text); font-variant-numeric: tabular-nums; padding-top: 1px;">{step.number}</span>
                <div style="display: grid; grid-template-columns: minmax(0, 1fr) 194px; gap: 28px; align-items: center;">
                  <div>
                    <h2 class="text-base leading-6 font-semibold" style="display: flex; align-items: center; gap: 9px; margin: 0 0 8px;">
                      <span style="display: flex; color: var(--ui-accent-link-text);"><step.icon size={19} aria-hidden="true" /></span>
                      {step.title}
                    </h2>
                    <p style="margin: 0; color: var(--ui-hoverable-text);">{step.description}</p>
                    <p class="text-xs leading-5" style="margin: 8px 0 0; color: var(--ui-muted-text);">{step.detail}</p>
                  </div>
                  <div style="padding: 14px; border: 1px solid var(--ui-accent-muted-border); border-radius: 9px; background: linear-gradient(145deg, var(--ui-accent-action-fill), var(--ui-card-normal-surface));">
                    {#if index === 0}
                      <div class="text-xs leading-5 font-semibold" style="display: flex; align-items: center; gap: 7px; color: var(--ui-normal-text);"><FolderOpen size={15} aria-hidden="true" />Product Development</div>
                      <div class="text-xs leading-6" style="margin: 8px 0 0 7px; padding-left: 14px; border-left: 1px solid var(--ui-accent-normal-border); color: var(--ui-muted-text);">Tasks/<br />Templates/</div>
                      <div class="text-xs leading-4" style="display: flex; align-items: center; gap: 5px; margin-top: 8px; color: var(--ui-accent-link-text);"><FileText size={12} aria-hidden="true" />Markdown files</div>
                    {:else if index === 1}
                      <div class="text-xs leading-5 font-semibold" style="color: var(--ui-normal-text);">Implement a feature</div>
                      <div class="text-xs leading-5" style="margin-top: 7px; color: var(--ui-muted-text);">Ask clarifying questions.<br />Propose a plan.<br />Implement and verify.</div>
                      <div class="text-xs leading-4" style="margin-top: 10px; padding-top: 8px; border-top: 1px dashed var(--ui-accent-normal-border); color: var(--ui-accent-link-text);">+ Your task</div>
                    {:else if index === 2}
                      <div class="text-xs leading-4" style="color: var(--ui-muted-text); margin-bottom: 8px;">Website / Improvements</div>
                      <div class="text-sm leading-5 font-semibold" style="color: var(--ui-normal-text);">Add keyboard navigation</div>
                      <div class="text-xs leading-4" style="display: flex; align-items: center; gap: 6px; margin-top: 12px; color: var(--ui-accent-link-text);"><span style="width: 6px; height: 6px; border-radius: 50%; background: var(--ui-accent-link-text);"></span>Todo</div>
                    {:else}
                      <div class="text-xs leading-5 font-semibold" style="display: flex; align-items: center; gap: 7px; color: var(--ui-accent-link-text);"><Zap size={15} aria-hidden="true" />Quick Template</div>
                      <div class="text-xs leading-5" style="margin-top: 7px; color: var(--ui-normal-text);">Implement a feature</div>
                      <div class="text-xs leading-4" style="display: flex; align-items: center; gap: 7px; margin-top: 11px; color: var(--ui-muted-text);"><Copy size={13} aria-hidden="true" />Copy<ArrowRight size={13} aria-hidden="true" />Your AI</div>
                      <div class="text-xs leading-4" style="display: flex; align-items: center; gap: 6px; margin-top: 12px; color: var(--ui-success-normal-text);"><Check size={14} aria-hidden="true" />Completed</div>
                    {/if}
                  </div>
                </div>
              </li>
      ...
      <div class="dialog-footer" style={dialog === 'welcome' ? 'align-items: center; flex-wrap: wrap;' : undefined}>
        {#if dialog === 'welcome'}<a class="welcomeGithubLink text-xs leading-5" href="https://github.com/coxthulhu/Cthulhu-Prompt/issues" target="_blank" rel="noreferrer" style="margin-right: auto; text-decoration: none;">Feedback &amp; support</a>{/if}
        {#if dialog === 'welcome'}
