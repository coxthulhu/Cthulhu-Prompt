...
{#snippet MovementButton(icon: Snippet, label: string, disabled = false, onclick?: () => void)}
  <button
    type="button"
    class="mockup-movement-button"
    aria-label={label}
    title={label}
    data-disabled={disabled ? 'true' : 'false'}
    disabled={disabled}
...
<style>
  ...
  .mockup-movement-button:last-child {
    border-bottom: 0;
  }
  ...
  .mockup-movement-button[data-disabled='true'] {
    cursor: default;
    opacity: 0.5;
    pointer-events: none;
  }
