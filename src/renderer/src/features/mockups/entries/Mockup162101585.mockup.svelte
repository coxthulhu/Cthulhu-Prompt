  ...
  <div class="dialog-layer text-base" role="presentation" use:mountDialog onclick={(event) => { if (event.target === event.currentTarget && dialog === 'error') closeDialog() }}>
    <div class="home-dialog" style={dialog === 'welcome' ? 'max-width: 900px; padding: 24px; border-radius: 16px;' : undefined} data-create={dialog === 'create'} data-welcome={dialog === 'welcome'} role="dialog" aria-modal="true" aria-label={dialog === 'create' ? 'Create Workspace' : dialog === 'welcome' ? 'Welcome' : 'Failed to Open Workspace'} tabindex="-1">
      <div class="dialog-header">
        <div class="dialog-heading">
          ...
          <div class="dialog-heading-text">
            ...
            {#if dialog === 'welcome'}<p class="text-sm leading-5" style="margin: 4px 0 0; color: var(--ui-muted-text);">Your first prompt, in four simple steps.</p>{/if}
      ...
      {:else if dialog === 'welcome'}
        <div class="welcomeContent text-sm leading-6" style="padding: 4px 0;">
          <ol style="list-style: none; padding: 0; margin: 0;">
            <li style="display: grid; grid-template-columns: 44px minmax(0, 1fr) 214px; gap: 18px; padding: 24px 4px; border-bottom: 1px solid var(--ui-neutral-muted-border);">
              <span class="text-2xl leading-8" style="color: var(--ui-accent-link-text); font-weight: 600; font-variant-numeric: tabular-nums;">01</span>
              <div>
                <h2 class="text-base leading-6 font-semibold" style="display: flex; align-items: center; gap: 9px; margin: 0 0 8px;"><Home size={18} />Make a workspace</h2>
                <p style="margin: 0; color: var(--ui-secondary-text);">Choose a folder on your computer. Your tasks and templates live here as Markdown files, ready to commit to source control.</p>
                <p class="text-xs leading-5" style="margin: 9px 0 0; color: var(--ui-muted-text);">Use the icons on the left to switch between screens.</p>
              </div>
              <div style="align-self: center; border: 1px solid var(--ui-neutral-normal-border); border-radius: 9px; padding: 13px; background: var(--ui-neutral-normal-surface);">
                <div class="text-sm leading-5 font-semibold" style="display: flex; gap: 8px; align-items: center;"><FolderOpen size={16} style="color: var(--ui-accent-link-text);" />Product Development</div>
                <div class="text-xs leading-5" style="margin: 10px 0 0 23px; color: var(--ui-secondary-text);">Tasks / fix-search.md<br />Templates / investigate.md</div>
              </div>
            </li>
            <li style="display: grid; grid-template-columns: 44px minmax(0, 1fr) 214px; gap: 18px; padding: 24px 4px; border-bottom: 1px solid var(--ui-neutral-muted-border);">
              <span class="text-2xl leading-8" style="color: var(--ui-accent-link-text); font-weight: 600; font-variant-numeric: tabular-nums;">02</span>
              <div>
                <h2 class="text-base leading-6 font-semibold" style="display: flex; align-items: center; gap: 9px; margin: 0 0 8px;"><Layers size={18} />Make a prompt template <span class="text-xs leading-4 font-normal" style="color: var(--ui-muted-text); border: 1px solid var(--ui-neutral-normal-border); border-radius: 5px; padding: 2px 6px;">Optional</span></h2>
                <p style="margin: 0; color: var(--ui-secondary-text);">Open Templates to write a reusable workflow: investigate a bug, plan a feature with Q&amp;A, or create mockups. Your task adds the specific instructions.</p>
              </div>
              <div style="align-self: center; border: 1px solid var(--ui-accent-muted-border); border-radius: 9px; padding: 13px; background: var(--ui-accent-action-fill);">
                <div class="text-xs leading-4" style="color: var(--ui-accent-link-text); margin-bottom: 7px;">PROMPT TEMPLATE</div>
                <div class="text-sm leading-5 font-semibold">Investigate a bug</div>
                <p class="text-xs leading-5" style="margin: 7px 0 0; color: var(--ui-secondary-text);">Find the cause. Explain the issue.<br />Propose a fix before editing.</p>
              </div>
            </li>
            <li style="display: grid; grid-template-columns: 44px minmax(0, 1fr) 214px; gap: 18px; padding: 24px 4px; border-bottom: 1px solid var(--ui-neutral-muted-border);">
              <span class="text-2xl leading-8" style="color: var(--ui-accent-link-text); font-weight: 600; font-variant-numeric: tabular-nums;">03</span>
              <div>
                <h2 class="text-base leading-6 font-semibold" style="display: flex; align-items: center; gap: 9px; margin: 0 0 8px;"><FileText size={18} />Make a prompt task</h2>
                <p style="margin: 0; color: var(--ui-secondary-text);">Open Tasks and describe the change you need. Add the context, requirements, and details your AI tool needs to do the work.</p>
                <p class="text-xs leading-5" style="margin: 9px 0 0; color: var(--ui-muted-text);">Name tasks, group them into categories, and track Backlog, Todo, In Progress, and Completed.</p>
              </div>
              <div style="align-self: center; border: 1px solid var(--ui-neutral-normal-border); border-radius: 9px; padding: 13px; background: var(--ui-neutral-normal-surface);">
                <div class="text-xs leading-4" style="display: flex; justify-content: space-between; color: var(--ui-muted-text); margin-bottom: 8px;"><span>Search improvements</span><span style="color: var(--ui-accent-link-text);">Todo</span></div>
                <div class="text-sm leading-5 font-semibold">Fix empty search results</div>
                <p class="text-xs leading-5" style="margin: 7px 0 0; color: var(--ui-secondary-text);">Searching by tag returns no results, even when tasks match.</p>
              </div>
            </li>
            <li style="display: grid; grid-template-columns: 44px minmax(0, 1fr) 214px; gap: 18px; padding: 24px 4px;">
              <span class="text-2xl leading-8" style="color: var(--ui-accent-link-text); font-weight: 600; font-variant-numeric: tabular-nums;">04</span>
              <div>
                <h2 class="text-base leading-6 font-semibold" style="display: flex; align-items: center; gap: 9px; margin: 0 0 8px;"><Zap size={18} />Pick a template. Copy and go.</h2>
                <p style="margin: 0; color: var(--ui-secondary-text);">Use Quick Template on a task to choose its template, or none, and copy the combined prompt. Paste it into your AI tool to get started.</p>
                <p class="text-xs leading-5" style="margin: 9px 0 0; color: var(--ui-muted-text);">When the work is done, check the task to move it to Completed.</p>
              </div>
              <div style="align-self: center; border: 1px solid var(--ui-success-muted-border); border-radius: 9px; padding: 13px; background: var(--ui-neutral-normal-surface);">
                <div class="text-xs leading-5" style="display: flex; align-items: center; justify-content: space-between; gap: 8px; color: var(--ui-secondary-text);"><span style="display: flex; gap: 7px; align-items: center;"><Layers size={14} />Template</span><span>+</span><span style="display: flex; gap: 7px; align-items: center;"><FileText size={14} />Task</span></div>
                <div class="text-sm leading-5 font-semibold" style="display: flex; align-items: center; justify-content: center; gap: 8px; border: 1px solid var(--ui-accent-normal-border); background: var(--ui-accent-action-fill); border-radius: 5px; padding: 8px; margin-top: 11px;"><Zap size={15} />Copy prompt<ArrowRight size={15} /></div>
                <div class="text-xs leading-5" style="display: flex; align-items: center; gap: 7px; margin-top: 9px; color: var(--ui-success-normal-text);"><Check size={14} />Ready for your AI tool</div>
              </div>
            </li>
          </ol>
        </div>
