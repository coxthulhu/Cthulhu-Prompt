<script lang="ts">
  ...
  type MockFolder = {
    id: string
    title: string
    chipAppearance: 'soft' | 'outline' | 'connected'
  ...
  let subfolders = $state<MockFolder[]>([
    ...
    },
    {
      id: 'base-release',
      title: 'Release',
      chipAppearance: 'connected',
      settings: createSettings(
        'base-release',
        'Prompts used to prepare and communicate a product release.',
        'Use the approved release notes and include only user-visible changes.'
      ),
      prompts: [
        createPrompt(
          'base-release-notes',
          ...
          'Release',
          [
            'Draft concise release notes for the completed work.',
            '',
            'Group changes by user impact and include any required migration steps.'
          ].join('\n')
        )
      ],
      children: []
      ...
      <div class="base-editor-body">
        ...
        <div class="base-folder-settings">
          <div class="rework-settings-toolbar">
            ...
            <div
              class="base-settings-rail"
              data-appearance={folder.chipAppearance}
              role="group"
...
<style>
  ...
  .base-settings-rail[data-appearance='soft'] button:hover:not([data-present='true']) {
    background: var(--ui-neutral-subtle-action-hover-fill);
    border-color: var(--ui-neutral-hover-border);
    color: var(--ui-normal-text);
  }

  .base-settings-rail[data-appearance='soft'] button[data-present='true'] {
    background: var(--ui-accent-action-fill);
    border-color: var(--ui-accent-normal-border);
    color: var(--ui-accent-normal-text);
  }

  .base-settings-rail[data-appearance='soft'] button[data-present='true'] .base-settings-pill-icon {
    background: var(--ui-accent-action-hover-fill);
    color: var(--ui-accent-normal-text);
  }

  .base-settings-rail[data-appearance='outline'] button {
    background: var(--ui-editor-content-surface);
    border-color: var(--ui-neutral-muted-border);
  }

  .base-settings-rail[data-appearance='outline'] button:hover:not([data-present='true']) {
    border-color: var(--ui-neutral-focus-border);
    color: var(--ui-normal-text);
  }

  .base-settings-rail[data-appearance='outline'] button[data-present='true'] {
    background: transparent;
  ...
  .base-settings-rail[data-appearance='outline'] button[data-present='true'] .base-settings-pill-icon {
    background: var(--ui-accent-action-fill);
    color: var(--ui-accent-normal-text);
  }

  .base-settings-rail[data-appearance='connected'] {
    background: var(--ui-editor-content-surface);
    border: 1px solid var(--ui-neutral-normal-border);
    border-radius: var(--cthulhu-ui-radius-control);
    gap: 0;
  ...
  .base-settings-rail[data-appearance='connected'] button {
    background: transparent;
    border: 0;
  ...
  .base-settings-rail[data-appearance='connected'] button:first-child {
    border-left: 0;
  }

  .base-settings-rail[data-appearance='connected'] button:hover:not([data-present='true']) {
    background: var(--ui-neutral-subtle-action-hover-fill);
    color: var(--ui-hoverable-text);
  }

  .base-settings-rail[data-appearance='connected'] button[data-present='true'] {
    background: var(--ui-neutral-emphasis-surface);
    color: var(--ui-normal-text);
  }

  .base-settings-rail[data-appearance='connected'] .base-settings-pill-icon {
    background: transparent;
    color: var(--ui-muted-icon-glyph);
    height: 16px;
    width: 16px;
  ...
  .base-settings-rail[data-appearance='connected'] button[data-present='true'] .base-settings-pill-icon {
    color: var(--ui-hoverable-icon-glyph);
  }
