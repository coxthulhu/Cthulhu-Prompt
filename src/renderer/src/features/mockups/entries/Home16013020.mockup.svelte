<script lang="ts">
  ...
  const shellStyle =
    'display:flex; flex:1 1 auto; min-width:0; overflow-y:auto; padding:24px; color:var(--ui-text);'
  const columnStyle =
  ...
  const titleStyle =
    'margin:0; color:var(--ui-text-bright); font-size:clamp(4rem, 9vw, 5.75rem); font-weight:700; letter-spacing:0.16em; line-height:0.92; text-align:center; white-space:nowrap; text-shadow:0 18px 44px rgba(0, 0, 0, 0.34);'
  const cardsStyle =
    'display:grid; gap:18px; grid-template-columns:repeat(auto-fit, minmax(320px, 1fr)); align-items:stretch;'
  const cardStyle =
    'display:flex; flex-direction:column; gap:18px; min-width:0; border:1px solid var(--ui-card-border-soft); border-radius:32px; padding:22px; background:linear-gradient(180deg, rgba(34, 37, 45, 0.92) 0%, rgba(16, 18, 24, 0.94) 100%); box-shadow:0 26px 68px rgba(0, 0, 0, 0.3), inset 0 1px 0 rgba(255, 255, 255, 0.04); backdrop-filter:blur(14px);'
  const headerRowStyle =
    'display:flex; align-items:flex-start; justify-content:space-between; gap:16px; flex-wrap:wrap;'
  const titleBlockStyle = 'display:flex; flex-direction:column; gap:4px; min-width:0;'
  const cardTitleStyle =
    'margin:0; color:var(--ui-text-bright); font-size:1.125rem; font-weight:600; letter-spacing:-0.025em;'
  const cardDescriptionStyle =
    'margin:0; color:var(--ui-text-muted); font-size:0.875rem; line-height:1.55;'
  ...
  const labelStyle =
    'color:var(--ui-text-muted); font-size:0.75rem; font-weight:600; letter-spacing:0.08em; text-transform:uppercase;'
  const statsGridStyle = 'display:grid; gap:14px; grid-template-columns:repeat(2, minmax(0, 1fr));'
  const statStyle =
    'display:flex; align-items:center; gap:14px; min-width:0; padding:18px; border:1px solid var(--ui-card-border-subcard); border-radius:24px; background:rgba(8, 10, 15, 0.72); box-shadow:inset 0 1px 0 rgba(255, 255, 255, 0.03);'
  const statIconStyle =
  ...
  const statLabelStyle =
    'color:var(--ui-text-muted); font-size:0.75rem; font-weight:600; line-height:1.05; letter-spacing:0.05em; text-transform:uppercase;'
  const statValueStyle =
    'color:var(--ui-text-bright); font-size:1.75rem; font-weight:700; line-height:1; font-variant-numeric:tabular-nums;'
  ...
  const modalStyle =
    'display:flex; flex-direction:column; gap:20px; width:min(100%, 520px); border:1px solid var(--ui-card-border-soft); border-radius:30px; padding:24px; background:linear-gradient(180deg, rgba(27, 30, 38, 0.98) 0%, rgba(14, 16, 22, 0.98) 100%); box-shadow:0 28px 72px rgba(0, 0, 0, 0.4), inset 0 1px 0 rgba(255, 255, 255, 0.04);'
  const footerStyle = 'display:flex; justify-content:flex-end; gap:12px; flex-wrap:wrap;'
  const secondaryModalButtonStyle =
    'display:inline-flex; align-items:center; justify-content:center; min-height:46px; padding:0 18px; border-radius:18px; border:1px solid var(--ui-border-default); background:rgba(255, 255, 255, 0.05); color:var(--ui-text); font-size:0.9rem; font-weight:600;'
  ...
  const checkboxRowStyle =
    'display:flex; align-items:flex-start; gap:12px; padding:16px 18px; border:1px solid var(--ui-card-border-subcard); border-radius:22px; background:rgba(9, 11, 16, 0.78);'
  ...
  <div style={columnStyle}>
    ...
    <div style={cardsStyle}>
      <section style={cardStyle}>
          ...
          <div
            style={`${statusBadgeBaseStyle} ${
              isWorkspaceReady
                ...
                : 'background:var(--ui-accent-surface); border-color:var(--ui-accent-border); color:var(--ui-accent-icon);'
            }`}
          >
            {#if isWorkspaceReady}
              <Check size={16} />
        ...
        <div
          style="display:flex; align-items:center; gap:12px; width:100%; min-width:0; padding:16px 18px; border:1px solid var(--ui-card-border-subcard); border-radius:24px; background:rgba(8, 10, 15, 0.78);"
        >
          <div
          ...
          <div style="display:flex; flex-direction:column; gap:4px; min-width:0;">
            ...
            <div
              ...
              style="color:var(--ui-text); font-size:0.875rem; font-weight:500; line-height:1.45; min-width:0; overflow:hidden; text-overflow:ellipsis; white-space:nowrap;"
      ...
      <section style={cardStyle}>
        ...
        <div style={actionsStackStyle}>
          ...
          {@render actionButton(
            ...
            FolderPlus,
            'border-color:var(--ui-border-default); background:rgba(255, 255, 255, 0.05); color:var(--ui-text);',
          ...
          {#if isWorkspaceReady}
            {@render actionButton(
              'Close Workspace',
              X,
              'border-color:rgba(255, 99, 71, 0.28); background:rgba(255, 99, 71, 0.12); color:var(--ui-text);',
    ...
    <div style={overlayStyle}>
      {#if dialogState === 'setup'}
        <section style={modalStyle}>
          <div style={titleBlockStyle}>
            <h2 style="margin:0; color:var(--ui-text-bright); font-size:1.35rem; font-weight:700;">
              Setup Workspace
            </h2>
          ...
          <label style={checkboxRowStyle}>
            ...
            />
            <span style="color:var(--ui-text); font-size:0.9rem; line-height:1.55;">
