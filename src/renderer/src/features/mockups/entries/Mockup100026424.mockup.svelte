<script lang="ts">
  ...
  let manageCategoriesOpen = $state(true)
  const categoryRows = [
    { name: 'Implementation', summary: 'Build and refine product changes', count: 8, folder: 'Product Work' },
    { name: 'Verification', summary: 'Tests, checks, and release readiness', count: 4, folder: 'Product Work' },
    { name: 'Research', summary: 'Explore approaches and constraints', count: 6, folder: 'Product Work' },
    { name: 'Code Review', summary: 'Review changes and address feedback', count: 5, folder: 'Engineering' },
    { name: 'Documentation', summary: 'Keep project guidance up to date', count: 3, folder: 'Engineering' },
    { name: 'Release Notes', summary: 'Summarize shipped improvements', count: 2, folder: 'Engineering' }
  ]
  const categoryFieldStyle = 'box-sizing: border-box; width: 100%; height: 38px; padding: 8px 11px; background: var(--ui-neutral-field-surface); color: var(--ui-normal-text); border: 1px solid var(--ui-neutral-normal-border); border-radius: var(--cthulhu-ui-radius-control);'
  // Side effect: size the shared dialog without introducing global mockup styles.
  const sizeCategoryDialog = (node: HTMLElement) => {
    const dialog = node.closest<HTMLElement>('[role="dialog"]')
    if (dialog) {
      dialog.style.width = '1080px'
      dialog.style.maxWidth = 'calc(100vw - 32px)'
    }
  }
  // Side effect: mount a fixed-height Monaco preview and dispose its model when the dialog closes.
  const categoryDescriptionEditor = (node: HTMLElement) => {
    const model = monaco.editor.createModel([
      '# Implementation', '',
      'Prompts for implementing approved product changes.', '',
      '## Working guidelines',
      '- Read the relevant code before making changes.',
      '- Follow existing component and data patterns.',
      '- Keep changes focused on the requested outcome.', '',
      '## Before handing off',
      'Describe the changes and the validation performed.',
      'Call out any remaining questions or follow-up work.'
    ].join('\n'), 'markdown')
    const editor = monaco.editor.create(node, {
      model, automaticLayout: true, fontSize: editorSizing.fontSize,
      lineHeight: lineHeightPx, lineNumbers: 'on', lineNumbersMinChars: 3,
      glyphMargin: false, minimap: { enabled: false }, wordWrap: 'on',
      scrollBeyondLastLine: false, overviewRulerBorder: false,
  ...
  <div class="base-content-viewport" bind:this={viewport} bind:clientHeight={viewportHeight} onscroll={updateBreadcrumb}>
    <section class="base-root-header">
      <div class="base-root-title-row">
        ...
        <div class="base-icon-button-bar">
          <button type="button" class="text-sm" style="display: flex; align-items: center; gap: 8px; padding: 9px 13px; border: 1px solid var(--ui-neutral-normal-border); border-radius: var(--cthulhu-ui-radius-control); background: var(--ui-neutral-action-fill); color: var(--ui-normal-text);" onclick={() => manageCategoriesOpen = true}><Settings size={17} />Manage Categories</button>
...
<Dialog bind:open={manageCategoriesOpen} icon={Settings} title="Manage Categories"
  subtitle="Organize categories and their settings across your workspace."
  submitText="Save changes" cancelText="Close" cancelFirst>
    <div use:sizeCategoryDialog style="display: grid; grid-template-columns: 280px minmax(0, 1fr); height: min(600px, calc(100vh - 210px)); min-height: 0; margin: 0 -16px; color: var(--ui-normal-text);">
      <aside style="display: flex; flex-direction: column; min-height: 0; border-right: 1px solid var(--ui-neutral-normal-border); background: var(--ui-card-normal-surface);">
        <div style="display: flex; align-items: center; justify-content: space-between; padding: 20px 18px 14px;">
          <span class="text-sm" style="font-weight: var(--font-weight-semibold);">Categories <span class="text-xs" style="margin-left: 5px; color: var(--ui-muted-text);">6</span></span>
          <button type="button" aria-label="Add category" class="text-xs" style="display: flex; align-items: center; gap: 5px; padding: 5px 8px; border: 1px solid var(--ui-neutral-normal-border); border-radius: var(--cthulhu-ui-radius-control); color: var(--ui-normal-text); background: var(--ui-neutral-action-fill);"><Plus size={14} />Add</button>
        </div>
        <div style="padding: 0 14px 14px;"><div style="display: flex; align-items: center; gap: 7px; border: 1px solid var(--ui-neutral-normal-border); border-radius: var(--cthulhu-ui-radius-control); padding: 8px 10px; background: var(--ui-neutral-field-surface); color: var(--ui-muted-text);"><Search size={15} /><input aria-label="Find categories" placeholder="Find categories…" class="text-sm" style="width: 100%; min-width: 0; border: 0; outline: none; color: var(--ui-normal-text); background: transparent;" /></div></div>
        <div style="min-height: 0; overflow-y: auto; padding: 0 8px 12px;">
          {#each ['Product Work', 'Engineering'] as folder (folder)}
            <div class="text-xs" style="display: flex; align-items: center; gap: 6px; padding: 10px 10px; color: var(--ui-secondary-text);"><Folder size={13} />{folder}</div>
            {#each categoryRows.filter((category) => category.folder === folder) as category (category.name)}
              <button type="button" aria-pressed={category.name === 'Implementation'} style="display: flex; width: 100%; text-align: left; gap: 9px; align-items: center; padding: 12px 10px; margin-bottom: 4px; border: 1px solid {category.name === 'Implementation' ? 'var(--ui-accent-muted-border)' : 'transparent'}; border-radius: var(--cthulhu-ui-radius-control); background: {category.name === 'Implementation' ? 'var(--ui-accent-action-fill)' : 'transparent'}; color: var(--ui-normal-text);">
                <Layers size={16} style="flex-shrink: 0; color: var(--ui-secondary-icon-glyph);" />
                <span style="display: grid; gap: 3px; min-width: 0; flex: 1;"><span class="text-sm" style="font-weight: var(--font-weight-semibold);">{category.name}</span><span class="text-xs" style="color: var(--ui-muted-text); overflow: hidden; text-overflow: ellipsis; white-space: nowrap;">{category.summary}</span></span>
                <span class="text-xs" style="color: var(--ui-muted-text);">{category.count}</span>
              </button>
            {/each}
          {/each}
        </div>
      </aside>
      <section aria-label="Category settings" style="min-width: 0; min-height: 0; overflow-y: auto; padding: 22px 26px;">
        <div style="display: flex; align-items: center; gap: 12px; margin-bottom: 22px;">
          <div style="display: grid; place-items: center; width: 40px; height: 40px; background: var(--ui-accent-action-fill); border: 1px solid var(--ui-accent-muted-border); border-radius: var(--cthulhu-ui-radius-control);"><Layers size={20} /></div>
          <div><h2 class="text-lg" style="margin: 0; font-weight: var(--font-weight-semibold);">Implementation</h2><p class="text-xs" style="margin: 3px 0 0; color: var(--ui-muted-text);">Product Work · 8 prompts</p></div>
        </div>
        <div style="display: grid; gap: 18px;">
          <div><label for="category424-name" class="text-sm" style="display: block; margin-bottom: 7px; font-weight: var(--font-weight-semibold);">Category name <span style="color: var(--ui-muted-text);">*</span></label><input id="category424-name" class="text-sm" value="Implementation" required aria-describedby="category424-name-help" style={categoryFieldStyle} /><p id="category424-name-help" class="text-xs" style="margin: 6px 0 0; color: var(--ui-muted-text);">Required. Use a unique name within Product Work; names are not case-sensitive.</p></div>
          <div><label for="category424-summary" class="text-sm" style="display: block; margin-bottom: 7px; font-weight: var(--font-weight-semibold);">Short description</label><input id="category424-summary" class="text-sm" value="Build and refine approved product changes." style={categoryFieldStyle} /><p class="text-xs" style="margin: 6px 0 0; color: var(--ui-muted-text);">A brief summary to help you recognize this category.</p></div>
          <div><div style="display: flex; align-items: center; justify-content: space-between; margin-bottom: 7px;"><span class="text-sm" style="font-weight: var(--font-weight-semibold);">Category description</span><span class="text-xs" style="color: var(--ui-muted-text);">Markdown</span></div><div style="height: 216px; overflow: hidden; border: 1px solid var(--ui-neutral-normal-border); border-radius: var(--cthulhu-ui-radius-control);"><div use:categoryDescriptionEditor style="height: 216px; width: 100%;"></div></div><p class="text-xs" style="margin: 6px 0 0; color: var(--ui-muted-text);">Describe the prompts in this category. For reference only; not included when copying prompts.</p></div>
          <div style="border-top: 1px solid var(--ui-neutral-normal-border); padding-top: 18px;"><h3 class="text-sm" style="margin: 0 0 14px; font-weight: var(--font-weight-semibold);">Prompt defaults</h3><div style="display: grid; grid-template-columns: 1fr 1fr; gap: 18px;">
            <div><label for="category424-status" class="text-sm" style="display: block; margin-bottom: 7px;">Initial status</label><select id="category424-status" class="text-sm" style={categoryFieldStyle}><option>Todo</option><option>In Progress</option><option>Backlog</option></select><p class="text-xs" style="margin: 6px 0 0; color: var(--ui-muted-text);">Status assigned to new prompts.</p></div>
            <div><label for="category424-template" class="text-sm" style="display: block; margin-bottom: 7px;">Default template</label><select id="category424-template" class="text-sm" style={categoryFieldStyle}><option>Draft Implementation Plan</option><option>No template</option></select><p class="text-xs" style="margin: 6px 0 0; color: var(--ui-muted-text);">Template selected for new prompts.</p></div>
          </div></div>
        </div>
      </section>
    </div>
