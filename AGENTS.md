...
- When launching subagents, do not fork the parent conversation context. Always set `fork_turns` to `"none"` and provide the necessary context in the subagent's task instructions.
- Whenever you run a custom script to change files, like a python script or node script, make sure to explain what that script did in your next update to the commentary channel.
...
- For control foregrounds, ghost and outline controls must brighten their text and glyphs on hover and focus, while filled and accented controls must keep their text and glyphs full white; semantic foregrounds such as warning and success must retain their semantic color.
